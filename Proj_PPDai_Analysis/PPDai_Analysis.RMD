...
```
用户每完成一项认证得一分，将所有的得分形成认证数新变量，进行数据统计
```{r}
# 计算认证数新变量
lc$auth <- 0
...
```{r}
# 不同借款期限的用户分布
# 借款金额数据比较分散,主要集中在6个月和12个月

lc$long.group <- cut(lc$'借款期限',
          c(0, 6, 12, 18, 24),
          labels = c('半年', '一年', '一年半', '两年'),
          right = T)

p1 <- ggplot(aes(x=lc$'借款期限'), data = lc) +
    geom_histogram(stat = "count", fill="#fab1a0") +
    labs(x="借款期限", y="用户数", title="借款期限用户分布") +
    theme(plot.title = element_text(hjust = 0.5, face = "bold", size = 12))

p2 <- ggplot(aes(x=long.group), data = lc) +
    geom_histogram(stat = "count", fill="#fab1a0") +
...
```{r, fig.width=8}
p1 <- ggplot(aes(x = lc$'借款利率'), data = lc) +
...
p2 <- ggplot(aes(y = lc$'借款利率'), data = lc) +
...
p2 <- ggplot(aes(y = lc$'历史成功借款次数'), data = lc) +
    geom_boxplot(color="#fab1a0") +
...
```{r}
p1 <- ggplot(aes(x = lc$'历史正常还款期数'), data = lc) +
    geom_histogram(stat = "count", fill="#fab1a0") +
    coord_cartesian(xlim = c(0, 40)) +
    labs(x="历史正常还款期数", y="用户数", title="历史正常还款期数用户数分布") +
    theme(plot.title = element_text(hjust = 0.5, face = "bold", size = 14))

p2 <- ggplot(aes(y = lc$"历史正常还款期数"), data = lc) +
    geom_boxplot(color="#fab1a0") +
    coord_cartesian(ylim = c(0, 20)) +
    labs(x="", y="历史正常还款期数", title="历史正常还款期数分布箱线图") +
    theme(plot.title = element_text(hjust = 0.5, face = "bold", size = 14))
...
```{r}
# 历史逾期还款期数用户分布
ggplot(aes(x = lc$'历史逾期还款期数'), data = lc) +
    geom_histogram(stat = "count", fill="#fab1a0") +
    scale_x_log10(breaks= c(0, 5, 15, 20, 40, 60)) +
    labs(x="历史正常还款期数", y="用户数", title="历史正常还款期数用户数分布") +
    theme(plot.title = element_text(hjust = 0.5, face = "bold", size = 14))
```
```{r}
...
ggplot(aes(x = lc$default_ratio), data = lc) +
    ...
    theme(plot.title = element_text(hjust = 0.5, face = "bold", size = 12))
```
违约概率为0的客户占比为84.7%,违约概率0.5的群体占比可达到95%，违约概率低于0.69的客户占比达到99%.
```{r}
summary(lc$LGD)
```
```{r}
...
- 在认证数的用户数量分布存在很大的差异，大多数用户只完成了1、2项，只有很少很少的用户完成了6项认证。
...
p1 <- ggplot(aes(x=initial_rating, y=mean),data = rate.initial_rating) +
    geom_bar(stat="identity", width = 0.4, fill="#fab1a0") +
    labs(x= "初始评级", y = "平均借款利率", title="不同初始评级的平均借款利率") +
    theme(plot.title = element_text(hjust = 0.5, face = "bold", size = 12))

p2 <- ggplot(aes(x=factor(auth), y=mean),data = rate.auth) +
    geom_bar(stat="identity", width = 0.4, fill="#fab1a0") +
    labs(x= "认证数", y = "平均借款利率", title="认证数的平均借款利率") +
    theme(plot.title = element_text(hjust = 0.5, face = "bold", size = 12))
...
ggplot(aes(x=age, y=default_ratio_mean),data = default_ratio.age) +
    geom_bar(stat="identity", width = 0.2, fill="#fab1a0") +
    labs(x= "年龄", y = "平均违约率", title="不同性别的平均违约率") +
    theme(plot.title = element_text(hjust = 0.5, face = "bold", size = 12))
...
p2 <- ggplot(aes(x=age.group, y=LGD_mean),data = LGD.age.group) +
    geom_bar(stat="identity", width = 0.2, fill="#fab1a0") +
    labs(x= "年龄分组", y = "平均违约损失率", title="不同年龄分组的平均违约损失率") +
    theme(axis.text.x = element_text(angle = 30, hjust = 1),
...
ggplot(aes(x=initial_rating, y=LGD_mean),data = LGD.initial_rating) +
    geom_bar(stat="identity", width = 0.3, fill="#fab1a0") +
    labs(x= "初始评级", y = "平均违约损失率", title="不同初始评级的平均违约违约损失率") +
    theme(plot.title = element_text(hjust = 0.5, face = "bold", size = 12))
...
ggplot(aes(x = age, y = mean_LGD), data = LDG.by_sex_age) +
    ...
    scale_x_continuous(breaks = seq(17, 60, 2)) +
    labs(x = "年龄", y = "违约损失率", title = "违约损失率和年龄、性别的关系") +
    theme(plot.title = element_text(hjust = 0.5, face = "bold", size = 12))
```
```{r echo=FALSE, fig.width=15, Multivariate_Plots1}
names(lc)[names(lc) == "性别"] <- "sex"
names(lc)[names(lc) == "借款成功日期"] <- "date"
...
ggplot(aes(x = date, y = accout_sum, size = count, color = sex), data = amout.date) +
    geom_point() +
    labs(x = "借款成功日期", y = "借款金额", color = "性别", size = "借款人数",
         title = "借款金额、人数随日期的变化关系") +
    theme(plot.title = element_text(hjust = 0.5, face = "bold", size = 12))
```
...
p2 <- ggplot(aes(x = age.group, fill=lc$'性别'), data = lc) +
    ...
    labs(x = "年龄分组", y = "用户数", fill = "性别", title = "不同年龄组用户数分布") +
    theme(plot.title = element_text(hjust = 0.5, face = "bold", size = 12),
...
grid.arrange(p1, p2, ncol = 2)
```
拍拍贷用户的年龄呈现左偏分布，取值范围位于17到56岁之间，将用户的年龄划分为小于18岁，18到22岁，22到25岁，25到28岁，28到35，35到40岁以及40岁以上几个区间段对用户数分析，其中28到35岁的用户数量时最多的，其次是25岁到28岁，再其次是22到25岁，而且小于18岁的用户数很少，在整个客户群体中几乎可以忽略，在不同年龄分组中男性的占比高于女性。
...
ggplot(aes(x=factor(auth)), data=lc)+
    ...
    theme(plot.title = element_text(hjust = 0.5, face = "bold", size = 12))
```
从认证数可以看出在统计的手机认证、户口认证、视频认证、学历认证、征信认证、淘宝认证中，绝大数无认证或者只完成了其中的0到2项认证，只有极少数的人完成了6项认证。
...
ggplot(aes(x = lc$'初始评级'), data = lc)+
    ...
    theme(plot.title = element_text(hjust = 0.5, face = "bold", size = 12))
```
用户的信用等级为D的用户最多，其次是C，这两者占用户信用等级的绝大多数，没有信用等级在AAA和AA的，从整体上来看用户的初始信用等级不高，需要加强用户信用等级的审核。
...
ggplot(aes(x = lc$'借款成功日期', y = default_ratio), data = lc) +
    ...
    geom_smooth() +
    labs(x = "借款成功日期", y = "违约率均值", title = "违约率均值随时间的变化") +
    theme(plot.title = element_text(hjust = 0.5, face = "bold", size = 14))
```
违约率不高，且无明显波动。
...
ggplot(aes(x = lc$'借款成功日期', y = LGD), data = lc) +
    geom_point(stat = "summary", fun.y = "mean", color = "#fab1a0") +
    geom_smooth() +
    labs(x = "借款成功日期", y = "违约损失率均值", title = "违约损失率均值随时间的变化") +
    theme(plot.title = element_text(hjust = 0.5, face = "bold", size = 14))
```
