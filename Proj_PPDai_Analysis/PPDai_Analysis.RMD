...
lc$month = month(lc$'借款成功日期')
lc2 <- rename(lc, 'load' = "借款金额", 'date' = "借款成功日期")
lc.month.group = lc2 %>%
        group_by(month) %>%
        summarise(sum = sum(load),     # 借款金额和
            count = n()                # 借款人数
        )
p1 <- ggplot(aes(x = month, y = sum), data = lc.month.group) +
    geom_point() +
    labs(x = "月份", y = "借款金额", title = "借款金额随时间的变化") +
    theme(axis.text.x = element_text(angle = 90, hjust = 1))

p2 <- ggplot(aes(x = month, weight = count), data = lc.month.group) +
    geom_bar() +
...
if(0){
    # 尝试将上述两图绘制在一个双轴图失败
    twoord.plot(lx=lc.month.group$date, ly=lc.month.group$sum,
            rx=lc.month.group$month, ry=lc.month.group$count,
