...
```{r}
# 安装和加载软件包
necessary <- c('ggplot2', 'ggthemes', "gridExtra", "dplyr", "alr3", "tidyr", "reshape2")
...
- [融合数据框](https://www.r-bloggers.com/melt/)
...
### 重塑数据
```{r, echo=FALSE}
pf.fc_by_age_gender.wide <- dcast(pf.fc_by_age_gender, age ~ gender, value.var = "fc_median")
head(pf.fc_by_age_gender.wide)
```

```{r, echo=FALSE}
pf.fc_by_age_gender.wide_array = acast(pf.fc_by_age_gender, age ~ gender, value.var = "fc_median")
...
df_facebook$year_joined.bucket <-  cut(df_facebook$year_joined, breaks = c(2004, 2009, 2011, 2012, 2014))
...
ggplot(aes(x = age, y = friend_count), 
       data =subset(df_facebook, !is.na(year_joined.bucket)),  df_facebook) +
